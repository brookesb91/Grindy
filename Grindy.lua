...
function Grindy:Handle(...)
  ...
  if amount > 0 then
    ...
    local change_color = Rate.Change == 0 and Colors.Blue or
      (Rate.Change < 0 and Colors.Red or Colors.Blue)

    Cache:Add(key, time)
    self:AddMessage("|C"..Colors.Blue..""..to_level_str.." more "..name.." to level.")
    self:AddMessage("|C"..Colors.Yellow.."Current Rate: "..rate_current_str.." xp/h |C"..change_color.."("..rate_change_str..")")
  end
end

function Grindy:AddMessage(message)
  DEFAULT_CHAT_FRAME:AddMessage("[Grindy] "..message.."")
end
